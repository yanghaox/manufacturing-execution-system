<template>
  <div class="pipeline-material-function">
    <!-- 表格 -->
    <plk-table
    ...
    >
      ...
      <template v-slot:pipelineId="row">
        <el-select v-if="row.scope.row.displayEdit" v-model="cachePipelineId" placeholder="请选择管道">
          <el-option
...
import PipelineMaterial from './js/PipelineMaterial.js'
...
import PipelineMaterialMixins from './mixins/PipelineMaterialMixins'
import { preventShake } from '@/utils/prevent-shake'
import { isNonnegativeNumber } from '@/utils/formRules'

export default {
  name: 'PipelineMaterialFunction',

  components: { PlkTable },

  mixins: [Emitter, PipelineMaterialMixins],
  ...
  methods: {
    ...
    init() {
      ...
      if (this.phaseStepType === TYPE.EDIT && this.functionalDomainData !== null) {
        const { materialId, currentProportion, scale, pipelineId } = this.currentFunctionData
        ...
        for (const item of this.materialListData) {
          if (materialId === item.materialId) {
            ...
            this.currentMaterialData = item
            break
          }
        }
        this.handleHideOtherButtonGroup()
      }
    },
    ...
    handleResetMaterialData(currentMaterialData) {
      const { materialId, currentProportion } = currentMaterialData
    ...
    handleTableRowSaveCommit({ row }) {
      ...
      row.displayButtonGroup = false

      this.currentMaterialData = row
      this.resetButtonDisabled = false
      this.handleCommitData(new PipelineMaterial(this.currentMaterialData))
