  ...
  <div class="material-container">
    <div class="warpper">
      <div class="container-header">
        ...
        <el-button
          v-permission-key="btnPermission.batchDeleteWip"
          type="danger"
          size="small"
        ...
        >
          <template v-slot:btnSlot="clo">
            ...
            <el-button
              ...
              type="primary"
              size="mini"
              @click="handleGetWiplDetail(clo)"
            >
              编辑
            </el-button>
            <el-button v-permission-key="btnPermission.deleteWip" type="danger" size="mini" @click="handleDelete(clo)">
              删除
            </el-button>
          </template>
        </plk-table>
...
const FORMOPTIONS = {
  ...
  materialCode: {
    label: '半成品代码',
    rules: [
      { required: true, message: '请输入半成品代码', trigger: 'blur' }
    ]
  },
  casNo: {
    label: '国际编号',
...
export default {
  ...
  data() {
    return {
      ...
      searchMaterialName: '',
      tableHeader: [
        { label: '半成品名称', prop: 'materialName' },
        { label: '半成品代码', prop: 'materialCode' },
  ...
  methods: {
    ...
    handleAddWip() {
      ...
      this.formDialog = {
        ...
        options: FORMOPTIONS,
        submit: this.handleSubmitWip
      }
    },

    // 提交半成品信息
    handleSubmitWip(value) {
    ...
    handleGetAnnexList(item) {
      getWipDetail(item.scope.row.materialId).then(res => {
        if (res.code === '200') {
    ...
    handleDelete(item) {
      this.$confirm(`确定删除${item.scope.row.materialCode}吗？`, '警告', {
        ...
        .then(() => {
          deleteWip(item.scope.row.materialId)
            .then(res => {
              if (res.code === '200') {
                this.$message.success(res.message)
                this.handleGetWipList()
    ...
    handleDeteleBatch() {
      ...
      this.$confirm('确定删除吗？', '警告', {
        ...
        .then(() => {
          batchDeletionWip(this.selectDataList)
            .then(res => {
              if (res.code === '200') {
                this.$message.success(res.message)
                this.handleGetWipList()
