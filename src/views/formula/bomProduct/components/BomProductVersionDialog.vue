<template>
  <el-dialog
    class="bom-product-version-dialog"
    :title="`${TITLE_MAP.get(bomVersionType)}成品BOM版本`"
    width="80%"
...
export default {
  ...
  data() {
    return {
      ...
      bomProductHeader: [
        // { label: '序号', prop: 'index' },
        { label: '包材代码', prop: 'materialCode', sortable: true, width: '120px' },
        { label: '包材名称', prop: 'materialName', width: '240px' },
  ...
  methods: {
    async init() {
      await this.getBomProductInfo()
      if (this.bomVersionType === TYPE.EDIT) {
        const bomVersionId = this.currentBomVersionData.bomVersionId
        this.getBomProductVersionDetail(bomVersionId)
      }
    },

    /* 获取成品BOM信息 */
    getBomProductInfo() {
      return getBomProductInfo().then(res => {
    ...
    /* 获取成品BOM版本详情 */
    getBomProductVersionDetail(bomVersionId) {
      return getBomProductVersionDetail(bomVersionId).then(res => {
    ...
    handleAddCommit(data) {
      ...
      this.$prompt('请输入成品bom版本', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPattern: /^\S+$/,
        inputErrorMessage: '请输入成品bom版本'
      }).then(({ value: version }) => {
        data['version'] = version
        return addBomProductVersion(data, bomId)
      }).then(res => {
    ...
    handleEditCommit(data) {
      ...
      this.$prompt('请输入成品bom版本', '提示', {
        ...
        data['version'] = version
        return editBomProductVersion(data, bomVersionId)
      }).then(res => {
        this.$message.success(res.data)
        this.handleBomVersionClose(bomId)
      })
    ...
    handleBomVersionCommit() {
      for (const item of this.bomProductData) {
        // 处理比例校验
      ...
      const mesFormulaMaterialBomVersionMaterialList = this.bomProductData.map(item => {
    ...
    handleTransferLeftRemove(materialSelectListNew) {
      ...
      for (const materialId of delMaterialKeys) {
        for (const [index, value] of this.bomProductData.entries()) {
          if (value.materialId === materialId) {
            this.bomProductData.splice(index, 1)
            break
          }
        }
      }
      ...
      for (const materialId of addMaterialKeys) {
        for (const item of this.materialData) {
          if (item.materialId === materialId) {
            ...
            this.bomProductData.push(material)
            break
          }
        }
      }
