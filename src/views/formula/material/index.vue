  ...
  <div class="material-container">
    <div class="warpper">
      <div class="container-header">
        ...
        <el-button v-permission-key="btnPermission.addMaterial" type="primary" size="small" @click="handleAddMaterial">
          新增
        </el-button>
        <el-button
          v-permission-key="btnPermission.batchDeleteMaterial"
          type="danger"
          size="small"
...
const FORMOPTIONS = {
  ...
  materialCode: {
    label: '原料代码',
    rules: [
      { required: true, message: '请输入原料代码', trigger: 'blur' }
    ]
  },
  casNo: {
    label: '国际编号',
...
export default {
  ...
  methods: {
    ...
    handleGetAnnexList(item) {
      getMaterialDetail(item.scope.row.materialId).then(res => {
        if (res.code === '200') {
    ...
    handlePutMaterial(value) {
      ...
      putMaterial(value, value.materialId)
        .then(res => {
          if (res.code === '200') {
            this.dialogVisible = false
            this.handleGetMaterialList()
    ...
    handleDelete(item) {
      this.$confirm(`确定删除${item.scope.row.materialCode}吗？`, '警告', {
        ...
        .then(() => {
          deletedMaterial(item.scope.row.materialId)
            .then(res => {
              if (res.code === '200') {
                this.$message.success(res.message)
                this.handleGetMaterialList()
