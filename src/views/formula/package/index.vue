  ...
  <div class="package-container">
    <div class="warpper">
      <div class="container-header">
        ...
        <el-button v-permission-key="btnPermission.addPachage" type="primary" size="small" @click="handleAddMaterial">
          新增
        </el-button>
        <el-button
          v-permission-key="btnPermission.batchDeletePachage"
          type="danger"
          size="small"
      ...
      <div class="package-table">
        ...
        >
          <template v-slot:btnSlot="clo">
            <el-button
              v-permission-key="btnPermission.editPachage"
              type="primary"
              size="mini"
            ...
            <el-button
              ...
              type="primary"
              size="mini"
              @click="handleGetMaterialDetail(clo)"
            >
              编辑
            </el-button>
            <el-button
              v-permission-key="btnPermission.deletePachage"
              type="danger"
              size="mini"
      ...
      <pagination
        ...
        :limit.sync="size"
        @pagination="handleGetPackageList"
      />
...
const FORMOPTIONS = {
  materialName: {
    label: '包材名称',
    rules: [
      { required: true, message: '请输入包材名称', trigger: 'blur' }
    ]
  },
  materialCode: {
    label: '包材代码',
    rules: [
      { required: true, message: '请输入包材代码', trigger: 'blur' }
    ]
  },
  casNo: {
    label: '国际编号',
...
export default {
  ...
  methods: {
    ...
    handleGetAnnexList(item) {
      getpackageDetail(item.scope.row.materialId).then(res => {
        if (res.code === '200') {
    ...
    handleAddMaterial() {
      ...
      this.formDialog = {
        ...
        options: FORMOPTIONS,
        submit: this.handleSubmitPackage
      }
    },

    // 提交包材信息
    handleSubmitPackage(value) {
    ...
    handlePutPackage(value) {
      ...
      putPackage(value, value.materialId)
        .then(res => {
          if (res.code === '200') {
            this.dialogVisible = false
            this.handleGetPackageList()
    ...
    handleDelete(item) {
      this.$confirm(`确定删除${item.scope.row.materialCode}吗？`, '警告', {
        ...
        .then(() => {
          deletePackae(item.scope.row.materialId)
            .then(res => {
              if (res.code === '200') {
                this.$message.success(res.message)
                this.handleGetPackageList()
    ...
    handleDeteleBatch() {
      ...
      this.$confirm('确定删除吗？', '警告', {
        ...
        .then(() => {
          batchDeletion(this.selectDataList)
            .then(res => {
              if (res.code === '200') {
                this.$message.success(res.message)
                this.handleGetPackageList()
