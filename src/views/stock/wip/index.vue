  ...
  <div class="material-container">
    <div class="warpper">
      <div class="container-header">
        ...
        <el-button
          v-permission-key="btnPermission.addWip"
          type="primary"
          size="small"
      ...
      <div class="material-table">
        ...
        >
          <template v-slot:btnSlot="clo">
            <el-button
              v-permission-key="btnPermission.editWip"
              type="primary"
              size="mini"
            ...
            <el-button
              v-permission-key="btnPermission.deleteWip"
              type="danger"
              size="mini"
...
import * as requestFunction from '@/api/stock/wip'
...
export default {
  ...
  data() {
    return {
      ...
      tableHeader: [],
      materialHeader: [
        { label: '半成品名称', prop: 'materialName', width: '100px' },
        { label: '半成品代码', prop: 'materialCode', width: '100px' },
      ...
      materialPackageHeader: [
        ...
        { label: '当前库存量（kg）', prop: 'amount', width: '140px' },
      ...
      materialData: [], // 半成品库存
      packageLogData: [], // 半成品包装库存日志数据
      materialPackageList: [], // 半成品包装库存
      selectMaterialList: [], // 勾选的半成品列表
      packageSelectList: [], // 勾选半成品包装库存
  ...
  methods: {
    ...
    async initFormOption() {
      ...
      this.formOptions = {
        material: {
          ...
          rolling: true,
          rules: [
            { required: true, message: '请选择半成品名称', trigger: 'change' }
          ]
        },
        materialCode: {
          label: '半成品代码',
    ...
    handleGetMaterialList() {
      ...
      requestFunction.getWipList(data).then(res => {
        if (res.code === '200') {
    ...
    async handleGetMaterialInfoList() {
      const materialInfoList = await requestFunction.getWipInfoList()
    ...
    handleSubmitMaterial(value) {
      ...
      unset(data, 'material')
      requestFunction.postWip(data).then(res => {
        if (res.code === '200') {
    ...
    handleGetMaterialDetail(item) {
      requestFunction.getWipDetail(item.scope.row.stockId).then(res => {
        if (res.code === '200') {
          const formDialog = {
            show: true,
            title: `修改半成品信息：${res.data.materialName}`,
            optionValue: {
              id: res.data.stockId,
    ...
    handlePutMaterial(value) {
      ...
      unset(data, 'material')
      requestFunction.putWip(data, data.id).then(res => {
        if (res.code === '200') {
          this.formDialog.show = false
          this.handleGetMaterialList()
    ...
    handleDelete(item) {
      this.$confirm(`确定删除${item.scope.row.materialCode}吗？`, '警告', {
        ...
        .then(() => {
          requestFunction.deleteWip(item.scope.row.stockId)
            .then(res => {
              if (res.code === '200') {
                this.$message.success(res.message)
                this.handleGetMaterialList()
