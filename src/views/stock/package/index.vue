  ...
  <div class="material-container">
    <div class="warpper">
      <div class="container-header">
        ...
        <el-button
          v-permission-key="btnPermission.addPackage"
          type="primary"
          size="small"
      ...
      <div class="material-table">
        ...
        >
          <template v-slot:btnSlot="clo">
            <el-button
              v-permission-key="btnPermission.editPackage"
              type="primary"
              size="mini"
            ...
            <el-button
              v-permission-key="btnPermission.deletePackage"
              type="danger"
              size="mini"
...
import * as requestFunction from '@/api/stock/package'
...
export default {
  ...
  data() {
    return {
      ...
      tableHeader: [],
      materialHeader: [
        { label: '包材名称', prop: 'materialName' },
        { label: '包材代码', prop: 'materialCode' },
      ...
      stockId: '', // 包材id
      packageInfo: {}, // 单条包材包装库存信息
      materialData: [], // 包材库存
      packageLogData: [], // 包材包装库存日志数据
      materialPackageList: [], // 包材包装库存
      selectMaterialList: [], // 勾选的包材列表
      packageSelectList: [], // 勾选包材包装库存
  ...
  methods: {
    ...
    handleGetMaterialList() {
      ...
      requestFunction.getPackageList(data).then(res => {
        if (res.code === '200') {
    ...
    async handleGetMaterialInfoList() {
      const materialInfoList = await requestFunction.getPackageInfoList()
    ...
    handleSubmitMaterial(value) {
      ...
      unset(data, 'material')
      requestFunction.postPackage(data).then(res => {
        if (res.code === '200') {
    ...
    handleGetMaterialDetail(item) {
      ...
      requestFunction.getPackageDetail(item.scope.row.stockId).then(res => {
        if (res.code === '200') {
          const formDialog = {
            show: true,
            title: `修改包材信息：${res.data.materialName}`,
            optionValue: {
              id: res.data.stockId,
    ...
    handlePutMaterial(value) {
      ...
      unset(data, 'material')
      requestFunction.putPackage(data, data.id).then(res => {
        if (res.code === '200') {
          this.formDialog.show = false
          this.handleGetMaterialList()
    ...
    handleDelete(item) {
      ...
      this.$confirm(`确定删除${item.scope.row.materialCode}吗？`, '警告', {
        ...
        .then(() => {
          requestFunction.deletePackage(item.scope.row.stockId)
            .then(res => {
              if (res.code === '200') {
                this.$message.success(res.message)
                this.handleGetMaterialList()
